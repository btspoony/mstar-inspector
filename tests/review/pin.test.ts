 ...
 * preinstalled @mstar-harness/cli (plan 66 Task 1 — supersedes the plan 48
 * Task 1 pin).
 *
 * POLICY CHANGE (plan 47, user instruction 2026-09-06; range carried forward
 * at `^3.8.1` by plan 66): the repo manifest
 * `@mstar-harness/engine` range is now `^3.8.1` — the only range allowed in
 ...
 * by bun.lock (packages row must resolve to exact 3.8.1) +
 * `bun install --frozen-lockfile` in the image build. The image CLI install
 * is exact too (`@mstar-harness/cli@3.8.1`) — the manifest range exception
 * does NOT extend to the image.
 *
 * These tests fail if the harness image ref drifts off the 3.8.1 commit, if
 * the manifest range or the lockfile resolution drifts, if the image CLI
 * install drifts off the exact 3.8.1 pin, loses its PATH exposure, or
...
/** Harness 3.8.1 git ref fetched into the sandbox image (plan 66 Task 1). */
const HARNESS_381_REF = "4c8fbb216c444df832b1943d68e4eee76261bbd2";
/** Superseded pre-3.8.1 refs that must no longer appear in the Dockerfile. */
...
describe("sandbox image harness pin", () => {
  ...
  test("Dockerfile fetches the 3.8.1 harness commit", () => {
    expect(dockerfile).toContain(`fetch --depth 1 origin ${HARNESS_381_REF}`);
  });
  ...
  test("Dockerfile comment records the tag v3.8.1 double-write", () => {
    expect(dockerfile).toContain("tag v3.8.1");
  });
});
...
describe("sandbox image CLI pin", () => {
  ...
  test("Dockerfile preinstalls @mstar-harness/cli at the exact 3.8.1 pin (bun global)", () => {
    // bun global install of the exact pin — image pinning discipline; the
    // repo manifest `^3.8.1` exception (plan 47 policy change) does NOT
    // extend to the image.
    expect(dockerfile).toContain("bun add --global @mstar-harness/cli@3.8.1");
  });
...
describe("engine dependency pin", () => {
  ...
  test("@mstar-harness/engine manifest declares the ^3.8.1 range (plan 47 policy change)", () => {
    expect(pkg.dependencies["@mstar-harness/engine"]).toBe("^3.8.1");
  });

  test("@mstar-harness/engine bun.lock packages row resolves to exact 3.8.1", () => {
    const lockfile = readFileSync(join(REPO_ROOT, "bun.lock"), "utf8");
    // The packages row names the RESOLVED version verbatim (exact 3.8.1, no
    ...
    expect(lockfile).toContain('"@mstar-harness/engine": ["@mstar-harness/engine@3.8.1"');
  });
  ...
  test("@mstar-harness/engine bun.lock workspaces row mirrors the manifest range", () => {
    ...
    // manifest.
    expect(lockfile).toContain('"@mstar-harness/engine": "^3.8.1"');
  });
});

describe("review skill version pin", () => {
  test("REVIEW_SKILL_VERSION carries the exact 3.8.1+4c8fbb21 value", () => {
    expect(REVIEW_SKILL_VERSION).toBe("3.8.1+4c8fbb21");
  });
