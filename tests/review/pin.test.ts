 ...
 * preinstalled @mstar-harness/cli (plan 48 Task 1 — supersedes the plan 47
 * Tasks 1–2 pin).
 *
 * POLICY CHANGE (plan 47, user instruction 2026-09-06; range carried forward
 * at `^3.6.3` by plan 48): the repo manifest
 * `@mstar-harness/engine` range is now `^3.6.3` — the only range allowed in
 ...
 * by bun.lock (packages row must resolve to exact 3.6.3) +
 * `bun install --frozen-lockfile` in the image build. The image CLI install
 * is exact too (`@mstar-harness/cli@3.6.3`) — the manifest range exception
 * does NOT extend to the image.
 *
 * These tests fail if the harness image ref drifts off the 3.6.3 commit, if
 * the manifest range or the lockfile resolution drifts, if the image CLI
 * install drifts off the exact 3.6.3 pin, loses its PATH exposure, or
...
/** Harness 3.6.3 git ref fetched into the sandbox image (plan 48 Task 1). */
const HARNESS_363_REF = "6cbeea67179a82483429cc6476111bfdbb959c7e";
/** Superseded pre-3.6.3 refs that must no longer appear in the Dockerfile. */
...
describe("sandbox image harness pin", () => {
  ...
  test("Dockerfile fetches the 3.6.3 harness commit", () => {
    expect(dockerfile).toContain(`fetch --depth 1 origin ${HARNESS_363_REF}`);
  });

  test("Dockerfile no longer references the superseded refs", () => {
    expect(dockerfile).not.toContain(SUPERSEDED_362_REF);
  ...
  test("Dockerfile comment records the tag v3.6.3 double-write", () => {
    expect(dockerfile).toContain("tag v3.6.3");
  });
});
...
describe("sandbox image CLI pin", () => {
  ...
  test("Dockerfile preinstalls @mstar-harness/cli at the exact 3.6.3 pin (bun global)", () => {
    // bun global install of the exact pin — image pinning discipline; the
    // repo manifest `^3.6.3` exception (plan 47 policy change) does NOT
    // extend to the image.
    expect(dockerfile).toContain("bun add --global @mstar-harness/cli@3.6.3");
  });
...
describe("engine dependency pin", () => {
  ...
  test("@mstar-harness/engine manifest declares the ^3.6.3 range (plan 47 policy change)", () => {
    expect(pkg.dependencies["@mstar-harness/engine"]).toBe("^3.6.3");
  });

  test("@mstar-harness/engine bun.lock packages row resolves to exact 3.6.3", () => {
    const lockfile = readFileSync(join(REPO_ROOT, "bun.lock"), "utf8");
    // The packages row names the RESOLVED version verbatim (exact 3.6.3, no
    // range prefix) — this is what `bun install --frozen-lockfile` in the
    // image build installs. A bad resolution (e.g. 3.6.2, or the range
    // leaking into the resolved name) fails this anchor.
    expect(lockfile).toContain('"@mstar-harness/engine": ["@mstar-harness/engine@3.6.3"');
  });
  ...
  test("@mstar-harness/engine bun.lock workspaces row mirrors the manifest range", () => {
    ...
    // manifest.
    expect(lockfile).toContain('"@mstar-harness/engine": "^3.6.3"');
  });
});

describe("review skill version pin", () => {
  test("REVIEW_SKILL_VERSION carries the exact 3.6.3+6cbeea67 value", () => {
    expect(REVIEW_SKILL_VERSION).toBe("3.6.3+6cbeea67");
  });
