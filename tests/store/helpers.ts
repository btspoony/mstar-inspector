 ...
 *   review_failures, 0011 webhook deliveries (plan 20), 0012 custom
 *   providers (plan 23), 0013 idx_findings_review_id, 0014
 *   idx_reviews_reviewed_at (plan 22) — 0011/0012 arrive via the
 *   integration merge), i.e. the schema that binds `reviews.app_id` (plan
 *   13, QC fix wave 1 F-001), so every test exercising the REAL store.put
 *   against production-shaped data runs on this one — and plan-14 fixtures
 *   get the per-App config tables without re-applying migrations by hand.
