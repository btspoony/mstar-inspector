...
Tailwind v4 CSS-first 配置下，两 类「源码看着对、测试全绿、页面却没有样式」的静默失败在本仓真实发生（design language v2 QC F-001 Critical；后续 polish QC F-001 Warning）：
...
2. **裸 `border`（width-only）的 border-color 落在 `currentColor`**。Tailwind preflight（`border: 0 solid`）+ 只写宽度/样式的 `border` utility → border-color 取继承文本色（`--card-fg` = gray-1000 全强度）。后续 polish 轮的 SectionCard secondary 档写了 `shadow-none` 以为「无影 + 细边框」，实际渲染出全页最重的边框，层级强调反转。
...
- 验收含新 utility 的变更，QA gate 固定带 built-CSS grep 项（已成惯例）。

## Extension（2026-09-12）：opacity 修饰符的**成功**路径与编译形态

本文记录的是两类失败；后续 opacity 扩展补齐了「theme key 存在时 opacity 修饰符如何工作」的实证面（AD-641 探针 + built-CSS kill proof）：
