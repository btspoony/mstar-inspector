...
The bridge approach let UI work land in five separate passes without a big-bang restyle: each page migrates in its own pass while sharing one token source. Generated-component discipline + exact pins keep the supply chain auditable (Workers image has no runtime network for UI deps anyway).
...
- **Motion tokens**: `--duration-fast/base/slow` + `--ease-*` with a token-level 1ms reduced-motion fold; raw `duration-200/300/500`/`ease-linear` faces all swept to token consumers (required sweep completed).
